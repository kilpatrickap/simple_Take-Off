...
class TakeOffList_Widget(QtWidgets.QWidget):
    ...
    def insert_data(self):
        ...
        if text:
            # Add the text and completion status as a child item of the parent root_item
            child_item = QtWidgets.QTreeWidgetItem(self.root_item)
            child_item.setText(0, text)
            child_item.setCheckState(0, QtCore.Qt.CheckState.Unchecked)  # Initialize as unchecked
            self.data.append((text, False))  # Add (text, False) to the data list for unchecked item
            self.lineEdit.clear()
    ...
    def update_database(self, file_name='takeOffList_DB.json'):
        tree_data = {}  # Dictionary to store the data
        for text, completed in self.data:
            tree_data[text] = completed  # Store text and completion status
    ...
    def load_database(self, file_name='takeOffList_DB.json'):
        ...
        try:
            ...
            self.data = [(text, completed) for text, completed in tree_data.items()]

            self.root_item.takeChildren()
            for text, completed in self.data:
                child_item = QtWidgets.QTreeWidgetItem(self.root_item)
                child_item.setText(0, text)
                child_item.setCheckState(0,
                                         QtCore.Qt.CheckState.Checked if completed else QtCore.Qt.CheckState.Unchecked)
                self.root_item.addChild(child_item)
                self.root_item.setExpanded(True)
    ...
    def mark_completed(self):
        ...
        if item:
            ...
            self.update_completion_status(item)
    ...
    def update_completion_status(self, item):
        text = item.text(0)
        completed = item.checkState(0) == QtCore.Qt.CheckState.Checked

        # Update the data list with the completion status
        for i, (item_text, _) in enumerate(self.data):
            if item_text == text:
                self.data[i] = (item_text, completed)
