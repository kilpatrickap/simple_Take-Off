...
class Tab_m_Widget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.groupBox_m.setGeometry(QtCore.QRect(10, 10, 790, 501))
        ...
        font.setFamily("Helvetica")
        font.setPointSize(12)
        self.tableWidget_m.setFont(font)
    ...
    def square(self):
        try:
            ...
            for row in range(self.tableWidget_m.rowCount()):
                for column in range(self.tableWidget_m.columnCount()):
                    item = self.tableWidget_m.item(row, column)
                    if item is not None and item.text() == "SUM":
                        ...
                        for col in range(self.tableWidget_m.columnCount()):
                            cell_item = self.tableWidget_m.item(row, col)
    ...
    def retranslateUi(self):
        ...
        self.label_code.setText(_translate("tabWidget_m", ""))
    ...
    def on_resize(self, event):
        ...
        if screen_width < 1920 or screen_height < 1080:
            ...
            current_geometry = self.groupBox_m.geometry()
            new_width = int(current_geometry.width() * reduction_factor * 1.3)
            new_height = int(current_geometry.height() * reduction_factor * 1.25)    # TODO: To make responsive
            new_geometry = QtCore.QRect(current_geometry.x(), current_geometry.y(), new_width, new_height)
            self.groupBox_m.setGeometry(new_geometry)
        ...
        for col, width in enumerate(column_widths):
            self.tableWidget_m.setColumnWidth(col, width)
